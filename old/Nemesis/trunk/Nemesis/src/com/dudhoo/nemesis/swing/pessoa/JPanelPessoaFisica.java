...
public class JPanelPessoaFisica extends JPanel{
	...
	jLabelNome.setBounds(new Rectangle(15, 10, 90, 20));
	jTextFieldNome.setBounds(new Rectangle(130, 10, 210, 20));
        jLabelRg.setText("Rg:");
	...
	jPanelSexo.setBounds(new Rectangle(130, 45, 210, 65));
	...
	if(this.pessoaFisica != null){
	    ...
	    if(this.pessoaFisica.isSexo()){
                this.jRadioButtonMasc.setSelected(true);
            }else{
                this.jRadioButtonFem.setSelected(true);
            }
            if(this.pessoaFisica.getPessoa() != null){
                this.jTextFieldNome.setText(pessoaFisica.getPessoa().getNome());
            }else{
	...
	model = new DefaultComboBoxModel(EstadoCivilConstants.values());
	...
	try{
		...
		this.jTextFieldConjuge.setText(pf.getNome());		
	    }
	}catch(Exception e){
	    throw e;
	}finally  {
