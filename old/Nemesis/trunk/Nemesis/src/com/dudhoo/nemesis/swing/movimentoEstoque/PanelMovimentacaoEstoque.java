...
public class PanelMovimentacaoEstoque extends JPanel{
    ...
    private TIPO tipo;
    private ComboTipoConta comboTipoConta1 = null;
	...
	long precoUnitario ;
        if(tipo == TIPO.ENTRADA){
            precoUnitario = Long.parseLong(this.jTextFieldPrecoUnit.getText());    
        }else{
            precoUnitario = prod.getPrecoVenda();    
        }
	...
	conta.setPessoa(pes);
        conta.setTipoConta(comboTipoConta1.getTipoConta());
	...
	if(movimentoEstoque == null){
	    ...
	    jCheckBoxPago.setSelected(movimentoEstoque.getConta().isContaPaga());
            comboTipoConta1.setTipoConta(movimentoEstoque.getConta().getTipoConta());
	}
    }

    private void jbInit() throws Exception{
	this.setLayout(null);
        comboTipoConta1 = new ComboTipoConta( (tipo.equals(TIPO.ENTRADA) ? false : true) );
	this.setSize(new Dimension(394, 292));
	jPanelObservacoes1.setBounds(new Rectangle(30, 185, 355, 100));
	...
	jCheckBoxPago.setBounds(new Rectangle(85, 100, 110, 20));
	...
	jLabel1.setText(tipo == TIPO.ENTRADA ? "Fornecedor:" : "Comprador:");
        jTextFieldPrecoUnit.setEditable(tipo == TIPO.ENTRADA ? true : false);
        if(tipo == TIPO.SAIDA){
            botaoFindProduto.setPrecoHolder(jTextFieldPrecoUnit);
        }
	...
	jLabelData.setBounds(new Rectangle(85, 130, 130, 20));
        comboTipoConta1.setBounds(new Rectangle(85, 155, 255, 20));
        comboTipoConta1.setToolTipText("Qual conta que vai ter o fluxo de caixa atualizado");
        jLabel5.setText("Conta:");
        jLabel5.setBounds(new Rectangle(5, 155, 65, 20));
